async function registerDevice(gameUid) {
    const response = await fetch(`https://multigolf.twostone.xyz/join_game_session/${gameUid}`)
...
async function gameHasStarted(gameUid) {
    const response = await fetch(`https://multigolf.twostone.xyz/game_started/${gameUid}`)
    const data = await response.json()
